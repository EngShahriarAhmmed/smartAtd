...
import IdCardView, {
  type IdCardInstitution,
  type IdCardStudent,
} from '@/components/IdCardView';
...
export default function StudentQRCardPage() {
  ...
  const hasMinimumFilter =
    searchInput.trim().length > 0 || !!filterClass || !!filterSection;
  ...
  async function loadIdCards(
    ...
    if (!search && !filterClass && !filterSection) {
      setMessage({
        ok: false,
        text: 'Please select at least one filter: Search, Class, or Section.',
      });
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/students/id-cards?${params.toString()}`);
      ...
      if (!res.ok) {
        ...
        setMessage({
          ok: false,
          text: data.error || 'Failed to generate ID cards.',
        });
        return;
      }
      ...
      setMessage({
        ok: true,
        text: `${data.cards?.length || 0} ID card(s) generated.`,
      });
    } catch {
      setCards([]);
      setMessage({
        ok: false,
        text: 'Unable to generate ID cards.',
      });
    } finally {
      setLoading(false);
    }
    ...
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Load students by filter and generate secure QR ID cards. QR contains
        ...
        <button
          type="button"
          onClick={() => loadIdCards(1)}
          disabled={loading || !hasMinimumFilter}
          className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {loading ? (
      ...
      <div className="card mb-4 p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative w-full sm:max-w-sm">
              ...
              <button
                ...
                onClick={() => loadIdCards(1)}
                disabled={loading || !hasMinimumFilter}
                className="absolute right-1 top-1/2 -translate-y-1/2 rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-bold text-white disabled:opacity-50"
              >
                Load
              </button>
            </div>
            ...
            {(searchInput ||
              ...
              <button
                ...
                disabled={loading}
                className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 transition hover:bg-slate-50 disabled:opacity-50"
              >
                Clear
              </button>
            )}
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-bold text-blue-700">
            <Users size={16} />
        ...
        {!hasMinimumFilter && !hasLoaded && (
          <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700">
            Please select at least one filter: Search, Class, or Section.
          </div>
        )}

        {(appliedSearch || filterClass || filterSection) && (
          <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
            <span className="font-bold">Loaded filter:</span>
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-4">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Front and back sides are ready for printing.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex min-h-[420px] items-center justify-center p-8 text-center">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Select a filter and click Load & Generate ID Cards.
              </p>
            </div>
          </div>
        ) : cards.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-2 p-2 2xl:grid-cols-2">
            {cards.map((card) => (
              <IdCardView
                key={card.student._id || card.student.id || card.student.studentId}
